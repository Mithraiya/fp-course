...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f = foldLeft (\b a -> b ++ f a) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = accumOptional Nil
    where
      accumOptional :: List a -> List (Optional a) -> Optional (List a)
      accumOptional acc Nil = Full acc
      accumOptional acc ((Full a) :. t) = accumOptional (acc ++ (a :. Nil)) t
      accumOptional _ (Empty :. _) = Empty
...
lengthGT4 = (> 4) . accum 0
  where
    accum :: Int -> List a -> Int
    accum n Nil = n
    accum n (h :. t) = if n == 4 then 5 else accum (n + 1) t
